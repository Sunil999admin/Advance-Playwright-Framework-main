...
Production-grade Playwright + TypeScript automation framework built by Pramod Dutta for The Testing Academy.

Playwright TypeScript Node License
...
TEST_AUTHOR=Pramod
API Testing
API coverage targets Restful Booker by default and runs through Playwright's APIRequestContext, not a browser page. Set TTA_ENV=api to resolve baseURL from API_BASE_URL:
...
TTA custom report overview for API and UI runs

Dedicated API Project
API specs live under src/tests/apiTests/ and run through the dedicated Playwright api project:

{
  name: 'api',
  testMatch: /src\/tests\/apiTests\/.*\.spec\.ts/,
}
Browser projects ignore API specs, so request-only tests are not duplicated across Chromium, Firefox, WebKit, or mobile browser projects.

API Learning Layers
The API examples are split into layers so the same Restful Booker workflow can grow from direct Playwright calls into reusable framework code:
...
JSONPath response queries	src/tests/apiTests/04_jsonpath_plus/	Query JSON responses with jsonpath-plus — root, child, recursive descent, wildcard, index, slice, and filtration. Ships a cheat sheet.
...
npx playwright test src/tests/apiTests/01_restfulbooker_raw/crud.spec.ts --project=api
For multi-step API flows, use test.describe.serial and a typed state object to pass values like auth tokens and booking IDs between tests:
...
JSONPath Queries (jsonpath-plus)
Concept: jsonpath-plus lets you pull values out of a JSON document with a single path expression instead of manual obj.a.b[0].c chaining. Every query returns an array of matches.
...
Full operator reference: jsonpath-cheatsheet.md. Runnable demo: jsonpath-queries.e2e.spec.ts.

JSON Schema Validation (Ajv)
Concept: Validate an API response against a Draft-07 JSON Schema with ajv + ajv-formats. validateSchema(schema, data) returns { valid, errors, errorText } so a single expect covers the entire response shape.
...
Runnable demo: create-booking-schema.spec.ts.
...
Q: What about credentials? A: They come from @config/credentials, which reads .env (see Environment Configuration).
...
The Custom TTA reporter captures test stdout/stderr from logger.info(...) and renders it in each expanded test under Test Logs. When a spec uses test.step(...), the same logs are also distributed into the matching step details, so API and UI flows show both the action title and the relevant log line in the report.
...
Per-step screenshots	visualStep() helper	every step (see visualStep)
...
Custom TTA Report — Visual Flow
The custom CustomReporter.ts produces a branded, real-time HTML report at tta-report/report_<timestamp>.html. For the end-to-end checkout test it shows the whole journey — every step with its console log, its own screenshot, and the run video.

Overview — stats dashboard, environment bar, and the filterable test table:

TTA report overview

End-to-end flow — expand the test row: each of the 6 steps shows its log line and a screenshot, followed by the screenshots gallery and the run video:

TTA report — e2e checkout flow
...
Augment Code	.augment-guidelines, .augment/rules/
Antigravity / Codex / Aider / Jules	AGENTS.md
...
test-quality-checks.md	Any change under src/tests/**
Phase 1 Walkthrough
Full prompt-by-prompt build log for Phase 1 lives at docs/phase1/prompts.md. Replay every step to recreate the framework from scratch.

Contributing
Fork
Branch (git checkout -b feat/my-thing)
Add tests + npm run typecheck && npm run lint
Commit + push
Open PR
